...
// ─────────────────────────────────────────────────────────────
//  Build snapshot for objective evaluation
// ─────────────────────────────────────────────────────────────

/// Intermediate owned data extracted from the cache for snapshot construction.
/// This avoids borrowing cache while we need to mutate it later.
...
// ─────────────────────────────────────────────────────────────
//  Implicit gradient: dK/d(node_pos) contribution
// ─────────────────────────────────────────────────────────────

/// Accumulate -λ^T (dK/d(node_pos)) u into grad_pos.
///
/// For bar element e connecting nodes i,j:
///   ke = (EA/L) * [T -T; -T T] where T = c*c^T, c = (xj-xi)/L
///
///   dke/d(xi_d) involves d(EA/L)/d(xi_d) * [T -T; -T T]
///                       + (EA/L) * d[T -T; -T T]/d(xi_d)
///
///   d(1/L)/d(xi_d) = c_d / L  (for xi), -c_d / L (for xj)
///   dc_a/d(xi_d) = -(delta_{ad} - c_a*c_d) / L  (for xi)
///   dc_a/d(xj_d) = +(delta_{ad} - c_a*c_d) / L  (for xj)
fn accumulate_dk_dpos(
    ...
    for e in 0..ne {
        ...
        // For each spatial direction d that we differentiate w.r.t.:
        for d in 0..3 {
            // Compute dke/d(xi_d) and dke/d(xj_d)
            // dke/d(xi_d) = -(EA/L^2) * c_d * ke_normalized + (EA/L) * dT_block/d(xi_d)
            // where ke_normalized = [T -T; -T T] (without EA/L factor)

            // Term 1: d(EA/L)/d(xi_d) * [T -T; -T T]
            // d(EA/L)/d(xi_d) = -EA/L^2 * dL/d(xi_d) = -EA/L^2 * (-c_d) = EA*c_d/L^2
            // (for xi: dL/dxi_d = -c_d, for xj: dL/dxj_d = +c_d)
            let d_ea_over_l_dxi = ea * c[d] / (l * l);
            let d_ea_over_l_dxj = -ea * c[d] / (l * l);

            // Term 2: (EA/L) * d[T -T; -T T]/d(xi_d)
            // dT_{ab}/d(xi_d) = d(c_a*c_b)/d(xi_d)
            //   = dc_a/d(xi_d) * c_b + c_a * dc_b/d(xi_d)
            // dc_a/d(xi_d) = -(delta_{ad} - c_a*c_d) / L
            // dc_a/d(xj_d) = +(delta_{ad} - c_a*c_d) / L

            // Compute lambda^T * (dke/d(xi_d)) * u  and  lambda^T * (dke/d(xj_d)) * u
            let mut lam_dke_u_xi = 0.0;
            let mut lam_dke_u_xj = 0.0;

            for a_idx in 0..6 {
                for b_idx in 0..6 {
                    // Sign pattern of ke block: s[a][b]
                    ...
                    // Term 2: (EA/L) * block_sign * dT_{ab}/d(xi_d)
                    let ad = a_idx % 3;
                    let bd = b_idx % 3;
                    ...
                    let dt_ab_dxj = -dt_ab_dxi; // opposite sign for xj
            ...
            // dJ/d(xi_d) -= lambda^T * (dke/d(xi_d)) * u
            grad_pos[ni * 3 + d] -= lam_dke_u_xi;
            grad_pos[nj * 3 + d] -= lam_dke_u_xj;
        }
    }
}

// ─────────────────────────────────────────────────────────────
//  Implicit gradient: dK/dA contribution
// ─────────────────────────────────────────────────────────────

/// Accumulate -λ^T (dK/dA_e) u into grad_area.
/// For bar: ke = (EA/L) * [T -T; -T T], so dke/dA = (E/L) * [T -T; -T T] = ke/A.
fn accumulate_dk_da(
    grad_area: &mut [f64],
    cache: &FeaCache,
    problem: &FeaProblem,
    ...
    for e in 0..ne {
        ...
        // dJ/dA_e -= lambda^T * (dke/dA_e) * u
        let sec_idx = props.section_idx;
        grad_area[sec_idx] -= lam_dke_u;
    }
}

// ─────────────────────────────────────────────────────────────
//  Self-weight gradient w.r.t. node positions
// ─────────────────────────────────────────────────────────────
...
fn accumulate_df_dpos(
    ...
    for e in 0..ne {
        ...
        // f_gravity at node = 0.5 * rho * A * L * g
        // df/d(xi_d) = 0.5 * rho * A * dL/d(xi_d) * g[d_grav]
        ...
        for d_pos in 0..3 {
            ...
            // Contribution to grad_pos via (df/dpos)^T * lambda
            for d_grav in 0..3 {
                let g = problem.gravity[d_grav];
                let df_dxi = rho_a_half * dl_dxi * g;
                let df_dxj = rho_a_half * dl_dxj * g;

                // (df/d(xi_d))^T * lambda: this affects all DOFs where f changes
                // The gravity load at node ni DOF d_grav changes by df_dxi
                // So (df/dpos)^T * lambda contribution:
                grad_pos[ni * 3 + d_pos] += df_dxi * lambda_full[ni * 3 + d_grav];
                grad_pos[ni * 3 + d_pos] += df_dxj * lambda_full[nj * 3 + d_grav];
                grad_pos[nj * 3 + d_pos] += df_dxj * lambda_full[ni * 3 + d_grav];
                grad_pos[nj * 3 + d_pos] += df_dxi * lambda_full[nj * 3 + d_grav];
            }
        }
    }
}
...
pub fn fea_value_and_gradient(
    ...
    // 4. Build per-element densities for snapshot
    let ne = problem.num_elements;
    ...
    // 6. Build snapshot, evaluate loss, and accumulate dJ/du — all before mutating cache
    ...
    // Accumulate dJ/du from objectives into a temporary buffer
    let mut grad_u_full = vec![0.0; n_total];
    for obj in &problem.objectives {
        obj.accumulate_grad_u(&mut grad_u_full, &snap);
    }

    // For compliance: dJ/du = 2*f = 2*rhs (in free-DOF space)
    // The compliance objective's accumulate_grad_u is a no-op, so we add 2*rhs here
    // by checking if any objective is compliance-type. This is handled via the
    // adjoint method: the grad_u we set here goes through K*lambda = grad_u.
    ...
    // Drop the snapshot — now we can mutate cache
    drop(snap);
    ...
    // 8. Adjoint solve: K lambda = dJ/du
    solve_adjoint(cache)?;
    ...
    // 10. Pack gradients back into theta-space
    offset = 0;
    if mask.node_positions {
        for &ni in &mask.node_indices {
            for d in 0..3 {
